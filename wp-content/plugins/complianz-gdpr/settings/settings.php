...
function cmplz_get_chunk_translations() {
	...
	foreach ( $files as $file ) {
		...
		wp_register_script( $chunk_handle, plugins_url( 'build/' . $file, __FILE__ ), array(), true );
...
function cmplz_plugin_admin_scripts() {
	...
	if ( version_compare( $wp_version, '6.6', '<' ) && ! wp_script_is( 'react-jsx-runtime', 'registered' ) ) {
		wp_enqueue_script(
			...
			array( 'react' ), // it is a polyfill for react JSX runtime, it depends on react
			CMPLZ_VERSION,
		);
	}
...
function cmplz_add_option_menu() {
	...
	$warnings         = COMPLIANZ::$admin->get_warnings(
		array(
			'plus_ones' => true,
		)
	);
...
function cmplz_do_action( $request ) {
	...
	switch ( $action ) {
		...
		case 'get_cookiebanner_required':
			...
			$data        = array( 'required' => $is_required );
			break;
		case 'reset_settings':
			$data = cmplz_reset_settings();
			break;
		default:
			$data = apply_filters( 'cmplz_do_action', array(), $action, $request );
	}
...
function cmplz_reset_settings() {
	...
	return array(
		'success' => true,
		'id'      => 'reset_settings',
		'message' => __( 'Data successfully cleared', 'complianz-gdpr' ),
	);
}
...
function cmplz_other_plugins_data( $slug = false ) {
	...
	foreach ( $plugins as $index => $plugin ) {
		...
		if ( $slug ) {
			...
			$star_rating = array(
				...
				'rating_count' => $plugin_info->num_ratings,
			);
		}
		...
		} elseif ( isset( $plugin['constant_premium'] ) ) {
	...
	if ( $slug ) {
		foreach ( $plugins as $key => $plugin ) {
			if ( $plugin['slug'] === $slug ) {
				if ( $star_rating ) {
					$plugin['star_rating'] = $star_rating;
				}
	...
	if ( $plugins[0]['pluginAction'] !== 'installed' ) {
		return array( 'plugins' => array( $plugins[0] ) );
	}

	// If first plugin is installed, return only the second one
	return array( 'plugins' => array( $plugins[1] ) );
}
...
function cmplz_sanitize_field( $value, string $type, string $id ) {
	switch ( $type ) {
		...
		case 'document':
			return in_array( $value, array( 'generated', 'custom', 'url', 'none' ) ) ? $value : false;
		case 'url':
		...
		case 'multicheckbox':
			if ( ! is_array( $value ) ) {
				$value = empty( $value ) ? array() : array( $value => 1 );
			}
