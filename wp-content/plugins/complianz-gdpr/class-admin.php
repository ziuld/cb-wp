	...
	class cmplz_admin {
		...
		public function no_security_plugin_active() {
			...
			if ( defined( 'rsssl_version' ) ) {
				return false; // really simple security
			}
			if ( defined( 'WORDFENCE_VERSION' ) ) {
				return false; // wordfence
			}
			if ( class_exists( 'ITSEC_Core' ) ) {
				return false; // ithemes
			}
			if ( class_exists( 'AIO_WP_Security' ) ) {
				return false; // All in one security
			}
			if ( class_exists( 'SG_Security' ) ) {
				return false; // Siteground security
			}
			if ( defined( 'DEFENDER_VERSION' ) ) {
				return false;
			}
			if ( defined( 'SUCURISCAN_INIT' ) ) {
				return false;
			}
			...
			if ( function_exists( 'GOTMLS_install' ) ) {
				return false; // anti malware security and brute force firewall
			}
		...
		public function enqueue_assets( $hook ) {
			...
			wp_enqueue_style( 'complianz-admin', $url, array( 'wp-components' ), filemtime( $path ) );
		}
		...
		public function show_admin_notice() {
			...
			$warnings = $this->get_warnings( array( 'admin_notices' => true ) );
			if ( count( $warnings ) == 0 ) {
				return;
			}
				...
				<div class="cmplz-admin-notice-container">
					...
					<div class="cmplz-admin-notice-content">
						...
						if ( isset( $warning['url'] ) ) {
							...
							?>
								<a href="<?php echo esc_url_raw( $warning['url'] ); ?>" <?php echo $target; ?> aria-label="<?php echo esc_attr( $aria_label ); ?>"><?php echo $link_text; ?></a>
								<?php
						}
						?>
						</p>
		...
		public function get_warnings( $args = array() ) {
			...
			if ( $args['progress_items_only'] ) {
				foreach ( $warnings as $id => $warning ) {
					// prevent notices on upgrade to 5.0
					if ( ! isset( $warning['include_in_progress'] ) ) {
						continue;
					}
		...
		private function validate_function( $func ) {
			...
			if ( strpos( $func, 'get_option_' ) !== false ) {
			...
			} elseif ( preg_match( '/get_value_(.*)==(.*)/i', $func, $matches ) ) {
				$fieldname = $matches[1];
				$value     = $matches[2];
				...
				$output = false;
			} elseif ( preg_match( '/(.*)->(.*)/i', $func, $matches ) ) {
